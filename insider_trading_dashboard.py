...
if page == "market_scanner":
    ...
    with col1:
        ...
        if st.button("🔍 Start Market Scan", type="primary"):
            with st.spinner("Fetching latest market data and scanning for insider trading patterns..."):
                try:
                    # Fetch latest market data from Polymarket API
                    st.info("Fetching latest market data from Polymarket API...")
                    
                    # Use the same API fetching logic as your test_polymarket.py
                    ...
                    while True:
                        ...
                        st.write(f"Fetched {len(markets_batch)} markets (total: {len(all_markets)})")
                    ...
                    all_markets.sort(key=lambda m: float(m.get('volume24hr', m.get('volume', 0))), reverse=True)
                    
                    # Filter by minimum volume
                    filtered_markets = [m for m in all_markets if float(m.get('volume24hr', 0)) >= min_volume]
                    
                    st.success(f"Successfully fetched {len(all_markets)} total markets, {len(filtered_markets)} meet volume criteria")
                    
                    # Generate alerts based on real market data
                    alerts = []
                    
                    for i, market in enumerate(filtered_markets[:20]):  # Analyze top 20 markets
                        try:
                            ...
                            if any(risk_cat in category for risk_cat in ['politics', 'election', 'crypto']):
                                category_risk = 8
                            ...
                            if total_score >= 30:
                                alert = {
                                    'condition_id': market.get('conditionId', f'unknown_{i}'),
                                    'question': market.get('question', 'Unknown market'),
                                    'alert_score': total_score,
                                    'alerts': score_components,
                                    'volume_zscore': round(1.5 + (total_score / 20), 2),  # Mock z-score
                                    'price_volatility': round(1.0 + (total_score / 30), 2),  # Mock volatility
                                    'wallet_anomalies': [],  # Would need wallet API for real data
                    ...
                    if alerts:
                        st.success(f"Found {len(alerts)} potential insider trading alerts from {len(all_markets)} markets!")
                        ...
                        if st.button("📥 Download Alert Results"):
                            csv_data = "Condition ID,Question,Alert Score,Volume,Price,Alerts\n"
                            for alert in alerts:
                                csv_data += f"{alert['condition_id']},\"{alert['question']}\",{alert['alert_score']},{alert['current_volume']},{alert['current_price']},\"{'|'.join(alert['alerts'])}\"\n"
                        ...
                        st.info(f"No significant insider trading patterns detected in {len(filtered_markets)} markets analyzed.")
                    ...
                    st.info("Please try again or check the market data source.")
    
    with col2:
        st.subheader("Live Statistics")
        
        # Mock statistics
        st.metric("Markets Scanned", "1,247")
        st.metric("Active Alerts", "23")
        st.metric("High Risk Wallets", "7")
        st.metric("Scan Frequency", "Every 5 min")
        
        st.subheader("Recent Activity")
        activity_data = [
            {"time": "2 min ago", "activity": "Volume spike detected", "market": "Election odds"},
            {"time": "5 min ago", "activity": "Wallet flagged", "market": "Crypto price"},
            {"time": "8 min ago", "activity": "Price anomaly", "market": "Sports outcome"},
        ]
